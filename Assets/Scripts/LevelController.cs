...
public class LevelController : Singleton<LevelController> {
    ...
    private float gameTime = Constants.GameLength;
    ...
    void startGame()
    {
        ...
        hasPower = false;
        powerCooldown = Random.Range(Constants.PowerMinCooldown, Constants.PowerMaxCooldown);
    }
    ...
    {
        Debug.Log("WL: " + winner.ToString() + ", " + loser.ToString());
    ...
    private void broadcastScores(int[] scores)
    {
        ...
        for (int i = 0; i < 4; i++) 
        {
            if (frogScores[i].first)
            {
                ...
                ranks[i] = frogScores[i].second.first;
                scores[i] = frogScores[i].second.second;
            }
            else
            {
                ranks[i] = -1;
        ...
        gameOverScreen.setRanks(ranks, scores);
	...
	void Update () {
        ...
        float timeLeft = (startTime + Constants.GameLength) - Time.time;
        ...
        else if (isStart)
        {
            if (_ready)
            {
                ...
                float timeToStart = (tapTime + Constants.ActiveTime) - Time.time;
                if (timeToStart > 0f)
                {
        ...
        else
        {
            ...
            if (timeLeft >= Constants.GameLength * 1f / 2f) count = Random.Range(3.5f, 9.5f);
            else if (timeLeft >= Constants.GameLength / 4f && timeLeft < Constants.GameLength * 1f / 2f) count = Random.Range(3.5f, 7.5f);
        ...
        if (_start && timeLeft<=Constants.GameLength/4f && !hasAccelerate && timeLeft>0)
        {
