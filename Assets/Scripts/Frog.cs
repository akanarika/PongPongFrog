...
public class Frog : MonoBehaviour {
	...
	void Update() {
        ...
        if ((transform.position - LevelController.Instance.center).magnitude > Constants.GameBoundary ||
            (inAir && Time.time > (launchTime + Constants.AirTime)))
        {
            Debug.Log("Reset");
            reset();
        }
    ...
    public void jump(int player)
    {
        ...
        {
            lastFrog = -1;
            if ((recoverTime + Constants.DownTime) < Time.time)
            {
                escaped = true;
            }
        }
        else if (!inAir && (recoverTime + Constants.DownTime < Time.time))
        {
            lastFrog = -1;
            ...
            rb.velocity = (direction * Constants.JumpSpeed);

			frogID = player - 1;
			SoundManager.Instance.SFXFrogJump(frogID);
